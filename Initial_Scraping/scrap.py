...
for player in players:
    ...
    if len(forename.group()) == 12:
        names.append((surname.group()[10:-1],value))
        count += 1
    elif len(surname.group()) == 11:
        names.append((forename.group()[11:-1],value))
        count += 1
    else:
        names.append((forename.group()[11:-1] + " " + surname.group()[10:-1],value))
        count += 1
...
with open('players.csv', 'w+') as csvfile:
    ...
    filewriter.writerow(['Name', 'Value (in Millions)'])
    for name in names:
        filewriter.writerow([name[0], name[1]])
